...
func (rs *RedisStore) Save(sid SessionID, sessionState interface{}) error {
	...
	rs.Client.Set(sid.getRedisKey(), sessState, rs.SessionDuration)
...
func (rs *RedisStore) Get(sid SessionID, sessionState interface{}) error {
	...
	//sessState, err := rs.Client.Do("Get", sid.getRedisKey()).Result()
	sessState, err := rs.Client.Get(sid.getRedisKey()).Result()
	if err != nil {
		if err == redis.Nil {
	...
	rs.Client.Set(sid.getRedisKey(), sessState, rs.SessionDuration)
	return json.Unmarshal([]byte(sessState), sessionState)
